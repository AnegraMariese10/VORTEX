#include "SateliteUdpBasicBurst.h"
...
EXECUTE_ON_STARTUP(
        ...
        e->insert(SateliteUdpBasicBurst::ONCE, "once");
        e->insert(SateliteUdpBasicBurst::PER_BURST, "perBurst");
        e->insert(SateliteUdpBasicBurst::PER_SEND, "perSend");
        );

Define_Module(SateliteUdpBasicBurst);

int SateliteUdpBasicBurst::counter;

simsignal_t SateliteUdpBasicBurst::outOfOrderPkSignal = registerSignal("outOfOrderPk");

SateliteUdpBasicBurst::~SateliteUdpBasicBurst()
{
    cancelAndDelete(timerNext);
}

void SateliteUdpBasicBurst::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        timerNext = new cMessage("SateliteUdpBasicBurstTimer");
    }
}

L3Address SateliteUdpBasicBurst::chooseDestAddr()
{
...
Packet *SateliteUdpBasicBurst::createPacket()
{
    char msgName[32];
...
void SateliteUdpBasicBurst::processStart()
{
...
void SateliteUdpBasicBurst::processSend()
{
...
void SateliteUdpBasicBurst::processStop()
{
    socket.close();
    socket.setCallback(nullptr);
}

void SateliteUdpBasicBurst::handleMessageWhenUp(cMessage *msg)
{
...
void SateliteUdpBasicBurst::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void SateliteUdpBasicBurst::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void SateliteUdpBasicBurst::socketClosed(UdpSocket *socket)
{
    if (operationalState == State::STOPPING_OPERATION)
        startActiveOperationExtraTimeOrFinish(par("stopOperationExtraTime"));
}

void SateliteUdpBasicBurst::refreshDisplay() const
{
...
void SateliteUdpBasicBurst::processPacket(Packet *pk)
{
...
void SateliteUdpBasicBurst::generateBurst()
{
...
void SateliteUdpBasicBurst::finish()
{
...
void SateliteUdpBasicBurst::handleStartOperation(LifecycleOperation *operation)
{
...
void SateliteUdpBasicBurst::handleStopOperation(LifecycleOperation *operation)
{
    if (timerNext)
...
void SateliteUdpBasicBurst::handleCrashOperation(LifecycleOperation *operation)
{
    if (timerNext)
        cancelEvent(timerNext);
    activeBurst = false;
